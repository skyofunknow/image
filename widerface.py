...
def writexml(filename, saveimg, bboxes, xmlpath):
    ...
    annotation = doc.createElement('annotation')  #创建XML文档元素
    doc.appendChild(annotation)

    folder = doc.createElement('folder')
    folder_name = doc.createTextNode('widerface')  #创建文本元素
    folder.appendChild(folder_name)
    annotation.appendChild(folder)

    filenamenode = doc.createElement('filename')   #文件名与widerface原文件名一致
...
rootdir = r"F:\opencv\wired_face"

gtfile = r"F:\opencv\wired_face\wider_face_split\wider_face_train_bbx_gt.txt";  #原始标注文件

im_folder = r"F:\opencv\wired_face\WIDER_train\images";  #原始图片信息

##这里可以是test也可以是val

fwrite = open(r"F:\opencv\wired_face\VOC\ImageSets\Main\train.txt", "w")  #voc 文件信息
...
with open(gtfile, "r") as gt:
    while(True):
        ...
        if gt_con is None or gt_con == "":
            break
        im_path = os.path.join(im_folder, gt_con)#图片文件路径+文件名
        print(im_path)
        ...
        # cv2.waitKey(0)
        numbox = int(gt.readline())  #该文件目标数量
        #numbox = 0
        bboxes = []
        for i in range(numbox):
            ...
            bbox = (infos[0] + off_w, infos[1] + off_h, infos[2], infos[3])#x y w h
        ...
        cv2.imwrite(r"{}\VOC\JPEGImages\{}".format(rootdir, filename), im_data)  #生成图片文件

        xmlpath = r"{}\VOC\Annotations\{}.xml".format(rootdir, filename.split(".")[0])#  xml文件  去掉.jpg
